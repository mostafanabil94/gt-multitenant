...
export class CreateBrandAdminTable1621270531691 implements MigrationInterface {
  private tableForeignKey = new TableForeignKey({
    name: "fk_tbl_brand_admin_tbl_brand_foreignKey",
  ...
  private tableForeignKey1 = new TableForeignKey({
    name: "fk_tbl_brand_admin_tbl_user_foreignKey",
    columnNames: ["user_id"],
  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    const ifExsist = await queryRunner.hasTable("brand_admin");
    if (!ifExsist) {
  ...
  public async down(queryRunner: QueryRunner): Promise<any> {
    await queryRunner.dropTable("brand_admin", true);
  }
}
