...
export class CreateBranchUserTable1621270542903 implements MigrationInterface {
  private tableForeignKey = new TableForeignKey({
    name: "fk_tbl_branch_user_tbl_branch_foreignKey",
  ...
  private tableForeignKey1 = new TableForeignKey({
    name: "fk_tbl_branch_user_tbl_user_foreignKey",
    columnNames: ["user_id"],
  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    const ifExsist = await queryRunner.hasTable("branch_user");
    if (!ifExsist) {
  ...
  public async down(queryRunner: QueryRunner): Promise<any> {
    await queryRunner.dropTable("branch_user", true);
  }
}
