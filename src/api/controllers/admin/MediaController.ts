...
export class MediaController {
  ...
  public async ObjectList(
    @QueryParam("folderName") folderName: string,
    @QueryParam("limit") limit: number,
    @QueryParam("marker") marker: string,
    @Req() request: any,
    @Res() response: any
  ): Promise<any> {
    console.log("working");
  ...
  public async ObjectCount(
    ...
    while (isTruncated) {
      try {
        const respons = await await this.s3Service.listBucker(
          limit,
          marker,
          folderName
        );
        count += respons.Contents.length + respons.CommonPrefixes.length;
        respons.CommonPrefixes.forEach((item) => {
  ...
  public async CreateFolder(
    @Body({ validate: true }) folderNameValidation: FolderNameRequest,
    @Req() request: any,
    @Res() response: any
  ): Promise<any> {
    console.log("working");
    ...
    if (env.imageserver === "s3") {
      ...
      val = await this.imageService.createFolder(
  ...
  public async uploadFile(
    @Body({ validate: true }) fileNameRequest: FileNameRequest,
    @Req() request: any,
    @Res() response: any
  ): Promise<any> {
    ...
    let val: any;
    if (env.imageserver === "s3") {
      val = await this.s3Service.imageUpload(
        path === "" ? name : path + name,
        base64Data,
        type
      );
    } else {
      val = await this.imageService.imageUpload(
        path === "" ? name : path + name,
        base64Data
      );
    }
   ...
   */
  @Get("/image-resize")
  public async image_resize(
    @QueryParam("width") width: string,
    @QueryParam("height") height: string,
    ...
    if (ext[1] === "jpg" || ext[1] === "jpeg" || ext[1] === "png") {
      let val: any;
      if (env.imageserver === "s3") {
        val = await this.s3Service.resizeImage(
          imgName,
          imgPath,
          widthString,
          heightString
        ...
        val = await this.imageService.resizeImage(
          imgName,
          imgPath,
          widthString,
          heightString
