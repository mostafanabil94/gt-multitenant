...
export class CustomerGroupController {
  ...
  public async createCustomerGroup(
    ...
    const customerGroupSaveResponse = await this.customerGroupService.create(
      newGroupParams
    );
  ...
  public async updateCustomerRole(
    ...
    const customerGroupSaveResponse = await this.customerGroupService.update(
      id,
      newCustomerGroup
    );
    if (customerGroupSaveResponse) {
  ...
  @Authorized()
  public async customergroupList(
    ...
    const customerGroupList = await this.customerGroupService.list(
      limit,
      offset,
      select,
      whereConditions,
  ...
  public async deleteGroup(
    @Body({ validate: true }) group: DeleteGroupRequest,
    @Res() response: any,
    @Req() request: any
  ): Promise<any> {
    ...
    if (findCustomer) {
      const errorResponse: any = {
        status: 0,
        message:
          "Users are mapped for this group, So you cannot delete this customerGroup",
      };
      return response.status(400).send(errorResponse);
    }
